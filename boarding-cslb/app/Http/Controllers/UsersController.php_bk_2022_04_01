...
class UsersController extends Controller
{
    ...
    public function load_user_group(Request $request) { 
        ...
        $arg_user =  explode(",", $list_user_id);
        $user_arg = array();
        $current_user = explode(",", $id_user);        
        foreach (DB::table('users')->get() as $user) {
            if(in_array($user->id, $current_user)) {
                if (in_array($user->id, $arg_user)) {
                    $user_html = '<li>
                            <label class="user-task">
                                <span class="name-user">'.$user->name.'</span>
                                <input type="checkbox" class="active" name="user_id" data-task_id="'.$_POST['task_id'].'" data-group_id="'.$_POST['group_id'].'" data-group_name="'.$_POST['group_name'].'" data-user_name="'.$user->name.'" value="'.$user->id.'" checked>
                            </label> 
                        </span>
                    </li>';
                    array_push($user_arg,$user_html);
                    ...
                    $user_html = '<li>
                        <label class="user-task">
                            <span class="name-user">'.$user->name.'</span>
                            <input type="checkbox" name="user_id" data-task_id="'.$_POST['task_id'].'" data-group_id="'.$_POST['group_id'].'" data-group_name="'.$_POST['group_name'].'" data-user_name="'.$user->name.'" value="'.$user->id.'">
                        </label> 
                    </span>
                </li>';   
                    array_push($user_arg,$user_html);                 
    ...
    public function open_user_group(Request $request) { 
        ...
        $arg_user =  explode(",", $list_user_id);
        $user_arg = array();
        $id_user = DB::table('group_users')->where('group_id',$group_id )->value('use_id');
        $current_user = explode(",", $id_user);
        $user_html=" ";  
        foreach (DB::table('users')->get() as $user) {
            if(in_array($user->id, $current_user)) {
