                      ...
                      <div class="collapse navbar-collapse" id="navbarTogglerDemo01">
                        ...
                        <ul class="navbar-nav align-items-center">
                            ...
                            @guest
                                ...
                                @endif
                            @else
                                @php if(Auth::check() && Auth::user()->is_admin == 1){ @endphp
                                <!-- <li class="nav-item">
...
<script type="text/javascript">
    jQuery(document).ready(function($){
        $(document).on('click','.list-group .edit-user', function(e){
            ...
            $.ajax({
                ...
                success: function(res) {
                    if(res.load_user_current) {
                        $('#popup-group-user .modal-body > ul').html(res.load_user_group);
                        $('#popup-group-user .modal-body .user-task-na input').attr('data-task_id', task_id);
                        if(res.na) {
                            $('.user-task-na input').prop('checked', true).addClass('active');
                        } else {
                            $('.user-task-na input').prop('checked', false).removeClass('active');
                        }
        ...
        $('.add-group').click(function(event) {
            ...
            var list_user = $('.input-group[data-id='+task_id+'] .current_user').val();
            var token = $('meta[name="csrf-token"]').attr('content');
            $.ajax({
                header:{
                    'X-CSRF-TOKEN': token
                },
                url: BASE_URL + "/open-popup",
                type: 'POST',
                dataType: 'json',
                data: {
                    list_user: list_user,
                ...
                success: function(res) {
                    $('#popup-group-user .modal-body > ul').html(res.html);
                    $('#popup-group-user').modal('show');
                    $('#popup-group-user .modal-body .user-task-na input').attr('data-task_id', task_id);
                    if(res.na) {
                        $('.user-task-na input').prop('checked', true).addClass('active');
        ...
        $(document).on('click','.select-action',function(event){
            $(this).next().slideDown();
            $(this).closest('li').siblings().find('.list-user').slideUp();
        });
    ...
    function clear_canvas(current) {
        ...
        var sigimage_exits = current.getAttribute("data-sigimage_exits");
        var canvas = document.getElementById(canvas_id);
        var sigText = document.getElementById(sigText_id);
        var sigImage = document.getElementById(sigImage_id);
        var sigimage_exits = document.getElementById(sigimage_exits);
        canvas.width = canvas.width;
        sigText.value = "";
        sigImage.setAttribute("src", "");
        sigImage.style.display = "none";
        sigimage_exits.style.display = "none";
    }
    function submit_canvas(current) {
        ...
        var tabID = current.getAttribute("data-tabID");
